...
public class FixedShot extends SequentialCommandGroup {
     ...
     */
    public FixedShot(Shooter shooter, WristJoint wrist, Indexer indexer, Intake intake, double topShooterSpeed, double bottomShooterSpeed,
        double shooterTolerance, Rotation2d targetAngle, Rotation2d angularTolerance, double indexerSpeed, double intakeSpeed, double clearanceTime)
    {
        addCommands(
            new ParallelCommandGroup(
                new NFRRotatingArmJointSetAngle(wrist, targetAngle, angularTolerance, 0, true),
                new RampShooterWithDifferential(shooter, () -> topShooterSpeed, () -> bottomShooterSpeed)
                    .until(() -> shooter.isAtSpeed(shooterTolerance))
            ),
