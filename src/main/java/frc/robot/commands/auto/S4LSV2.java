...
public class S4LSV2 extends SequentialCommandGroup{
    protected static final PathPlannerPath[] paths = new PathPlannerPath[] {
        PathPlannerPath.fromPathFile("P1 (S4 to G7)"),
        PathPlannerPath.fromPathFile("P2 (G7 to score)"),
        PathPlannerPath.fromPathFile("P3 (G7 score to G6)"),
        PathPlannerPath.fromPathFile("P4 (G6 score)"),
        PathPlannerPath.fromPathFile("P5 (G5 from G6 score)"),
        PathPlannerPath.fromPathFile("P6 (G5 to score)")
    };
    /**
     * Creates S4LSV2 auto path
     ...
     */
    public S4LSV2(NFRSwerveDrive drive, NFRSwerveModuleSetState[] setStateCommands, Supplier<Pose2d> poseSupplier,
    ...
    {
        return new AutonomousRoutine(S4LSV2.class.getSimpleName(),
            () -> shouldFlipPath.getAsBoolean() ? paths[0].flipPath().getPreviewStartingHolonomicPose() : paths[0].getPreviewStartingHolonomicPose(),
            new S4LSV2(drive, setStateCommands, poseSupplier, controller, shouldFlipPath, ignoreCommands));
    }
}
