...
public class S3CV2 extends SequentialCommandGroup
{
    protected static final PathPlannerPath[] paths = new PathPlannerPath[] {
        PathPlannerPath.fromPathFile("S3.C.V2.1"),
        PathPlannerPath.fromPathFile("S2.C.V2.2"),
        PathPlannerPath.fromPathFile("S2.C.V2.3"),
        PathPlannerPath.fromPathFile("S2.C.V2.4"),
        PathPlannerPath.fromPathFile("S2.C.V2.5"),
        PathPlannerPath.fromPathFile("S2.C.V2.6")
    };
    /**
     * Creates a new S3CV2
     ...
     */
    public S3CV2(NFRSwerveDrive drive, NFRSwerveModuleSetState[] setStateCommands, Supplier<Pose2d> poseSupplier,
    ...
    /**
     * Gets the AutonomousRoutine struct for S3CV2
     * @param drive the drive subsystem
     * @param setStateCommands the commands to run each module
     * @param poseSupplier the supplier for pose estimation
     * @param controller the controller for following the path
    ...
    {
        return new AutonomousRoutine(S3CV2.class.getSimpleName(),
            () -> shouldFlipPath.getAsBoolean() ? paths[0].flipPath().getPreviewStartingHolonomicPose() : paths[0].getPreviewStartingHolonomicPose(),
            new S3CV2(drive, setStateCommands, poseSupplier, controller, shouldFlipPath));
    }
    /**
     * Gets the AutonomousRoutine struct for S3CV2
     * @param drive the drive subsystem
        ...
        WristJoint wrist, Intake intake)
    {
        return new AutonomousRoutine(S3CV2.class.getSimpleName(),
            () -> shouldFlipPath.getAsBoolean() ? paths[0].flipPath().getPreviewStartingHolonomicPose() : paths[0].getPreviewStartingHolonomicPose(),
            new S3CV2(drive, setStateCommands, poseSupplier, controller, shouldFlipPath, shooter, wrist, intake));
    }
}
