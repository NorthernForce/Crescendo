...
public class S2CSV2 extends SequentialCommandGroup{
    protected static final PathPlannerPath[] paths = new PathPlannerPath[] {
        PathPlannerPath.fromPathFile("P1 (S2 to G2)"),
        PathPlannerPath.fromPathFile("P2 (G2 to G3)"),
        PathPlannerPath.fromPathFile("P3 (G3 to G6)"),
        PathPlannerPath.fromPathFile("P4 (G6 to score)"),
        PathPlannerPath.fromPathFile("P5 (G6 score to G5)"),
        PathPlannerPath.fromPathFile("P6 (G5 score)")
    };
    /**
     * Creates S2CSV2 auto path
     ...
     */
    public S2CSV2(NFRSwerveDrive drive, NFRSwerveModuleSetState[] setStateCommands, Supplier<Pose2d> poseSupplier,
    ...
    {
        return new AutonomousRoutine(S2CSV2.class.getSimpleName(),
            () -> shouldFlipPath.getAsBoolean() ? paths[0].flipPath().getPreviewStartingHolonomicPose() : paths[0].getPreviewStartingHolonomicPose(),
            new S2CSV2(drive, setStateCommands, poseSupplier, controller, shouldFlipPath, ignoreCommands));
    }
}
