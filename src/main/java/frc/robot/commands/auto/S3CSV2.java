...
public class S3CSV2 extends SequentialCommandGroup{
    protected static final PathPlannerPath[] paths = new PathPlannerPath[] {
        PathPlannerPath.fromPathFile("Copy of P1 (S2 to G2)"),
        PathPlannerPath.fromPathFile("Copy of P2 (G2 to G3)"),
        PathPlannerPath.fromPathFile("Copy of P3 (G3 to G6)"),
        PathPlannerPath.fromPathFile("Copy of P4 (G6 to score)"),
        PathPlannerPath.fromPathFile("Copy of P5 (G6 score to G5)"),
        PathPlannerPath.fromPathFile("Copy of P6 (G5 score)")
    };
    /**
     * Creates S3CSV2 auto path
     ...
     */
    public S3CSV2(NFRSwerveDrive drive, NFRSwerveModuleSetState[] setStateCommands, Supplier<Pose2d> poseSupplier,
    ...
    {
        return new AutonomousRoutine(S3CSV2.class.getSimpleName(),
            () -> shouldFlipPath.getAsBoolean() ? paths[0].flipPath().getPreviewStartingHolonomicPose() : paths[0].getPreviewStartingHolonomicPose(),
            new S3CSV2(drive, setStateCommands, poseSupplier, controller, shouldFlipPath, ignoreCommands));
    }
}
