...
public class S2CV1 extends SequentialCommandGroup
{
        ...
        Supplier<Pose2d> poseSupplier, PPHolonomicDriveController controller, BooleanSupplier shouldFlipPath)
    {
        return new AutonomousRoutine(S2CV1.class.getSimpleName(),
            ...
            new S2CV1(drive, setStateCommands, poseSupplier, controller, shouldFlipPath));
    }
    /**
     * Gets the AutonomousRoutine struct for S2CV1
     * @param drive the drive subsystem
        ...
        WristJoint wrist, Intake intake)
    {
        return new AutonomousRoutine(S2CV1.class.getSimpleName(),
            () -> shouldFlipPath.getAsBoolean() ? paths[0].flipPath().getPreviewStartingHolonomicPose() : paths[0].getPreviewStartingHolonomicPose(),
            new S2CV1(drive, setStateCommands, poseSupplier, controller, shouldFlipPath, shooter, wrist, intake));
    }
}
