...
public class TurnToCoordinates extends Command
{
    ...
    public TurnToCoordinates(NFRSwerveDrive drive, NFRSwerveModuleSetState[] setStateCommands, PIDController controller,
        DoubleSupplier xSupplier, DoubleSupplier ySupplier, Supplier<Pose2d> poseSupplier, Supplier<Translation2d> targetCoordinates,
    ...
    public void execute()
    {
        ...
        var coords = targetCoordinates.get();
        double targetAngle = Rotation2d.fromRadians(Math.atan2(coords.getY() - pose.getY(), coords.getX() - pose.getX())).minus(pose.getRotation()).getRadians();
        ChassisSpeeds speeds = new ChassisSpeeds(xSupplier.getAsDouble(), ySupplier.getAsDouble(), controller.calculate(targetAngle));
        if (fieldRelative)
