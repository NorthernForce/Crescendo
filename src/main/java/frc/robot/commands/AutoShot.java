...
public class AutoShot extends SequentialCommandGroup {
    public AutoShot(SwerveDrive drive, NFRSwerveModuleSetState[] setStateCommands, Intake intake, Indexer indexer, WristJoint wrist,
        DoubleSupplier xSupplier, DoubleSupplier ySupplier, DoubleSupplier thetaSupplier, TargetCamera camera, Shooter shooter,
        DoubleSupplier lastRecordedDistance, TargetingCalculator topSpeedCalculator, TargetingCalculator bottomSpeedCalculator,
        TargetingCalculator wristCalculator)
    {
        addCommands(
            new ParallelCommandGroup(
                new TurnToTarget(drive, setStateCommands, CrabbyConstants.DriveConstants.turnToTargetController, xSupplier, ySupplier, thetaSupplier,
                    () -> camera.getSpeakerTag(), true, true),
